...
function StockSpam(stockes){
    let result = [];

    for(let i = 0; i < stockes.length; i++){
        let count = 1;

        // Go LEFT OF THE CURRENT ELEMENT
        for(let j = i - 1; j >= 0; j--){
            if(stockes[i] >= stockes[j]){
                count++;
            }else{
                break;
            }
...
// console.log(StockSpam([100, 80, 90, 120])) // [ 1, 1, 2, 4 ]
// console.log(StockSpam([10, 4, 5, 90, 120, 80])) // [ 1, 1, 2, 4, 5, 1 ]

// T.C -> O(n^2)

// I use stack and what i do i just user push and pop i pust only index idx
...
console.log(BetterApproach([100, 80, 90, 120])) // [ 1, 1, 2, 4 ]
console.log(BetterApproach([10, 4, 5, 90, 120, 80])) // [ 1, 1, 2, 4, 5, 1 ]